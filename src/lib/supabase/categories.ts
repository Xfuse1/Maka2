...
export async function getActiveCategories() {
  ...
  if (error) {
    ...
    if (msg.includes("Failed to fetch") || msg.includes("NetworkError")) {
      console.warn(
        "[v0] Unable to contact Supabase from the browser. Check NEXT_PUBLIC_SUPABASE_URL/NEXT_PUBLIC_SUPABASE_ANON_KEY and network access.",
      )
      console.debug("Supabase client error:", msg)
    } else {
      console.error("[v0] Error fetching active categories:", msg)
    }
